...
- **Solo** : jeux quotidiens accessibles direct via `/play/[slug]`. Un puzzle par jour, identique pour tout le monde (seed = jour UTC), progression en localStorage uniquement, pas de compte.
...
lib/
  ...
  quizCeo/                       — config (16 types), types (discriminated union payload/answer), serialize (strip answer en phase playing), lolChampion (helper runtime DL splash + sorts), textQuestions (1500 questions ouvertes EASY/MEDIUM/HARD)
  solo/dailyIndex.ts             — dailyIndex(), pickByDay, seededShuffle, mulberry32, dailyDateKey
  motus/words.ts                 — MOTUS_CLEAN_WORDS + normalizeMotus (strip accents + upper)
  worldle/countries.ts           — COUNTRIES + haversineKm + bearingDeg + arrowForBearing + findCountry
  wikiera/entries.ts             — WIKIERA_ENTRIES (texte + aliases) + matchesWikiera
  password/rules.ts              — RULES[] + buildDailyContext (dayOfMonth, monthIndex, weekdayFr)
  cemantix/puzzles.ts            — PUZZLES (tier1..4) + scoreGuess + tierLabel
...
- **`react-hooks/set-state-in-effect` est strict** dans ce projet. Ne JAMAIS faire `setState` directement dans un `useEffect` body pour hydrater depuis un store externe (localStorage). Deux options conformes :
  ...
  2. **"derived state from previous render"** (pattern React docs) : `const [last, setLast] = useState(...)` + `if (last !== current) { setLast(current); setOther(...); }` dans le corps du composant. Cf. `PlayingView.tsx` Quiz CEO.
...
- **16 types de questions** tirées parmi ceux que le créateur active dans le lobby :
  `image-personality`, `text-question`, `country-shape`, `expression`, `music`, `translation`, `multiple-choice`, `odd-one-out`, `media-image`, `country-motto`, `brand-logo`, `absurd-law`, `price`, `who-said`, `ranking`, `lol-champion`.
...
### Sécurité anti-spoil
`GET /api/rooms/[code]` applique 2 filtres quand `phase ∈ {playing, waiting_review}` :
1. Strip du champ `answer` dans chaque question du snapshot.
2. Chaque joueur ne voit QUE ses propres `answers` (les autres sont mises à `[]`).
...
- `start` (créateur) : tire N questions, re-shuffle `shuffledOrder` pour `ranking`, crée QuizCeoGame + states, `gameStarted=true`.
- `next` (tous, idempotent) : avance `currentIndex` quand timer expiré ; dernière question → `phase=waiting_review`.
- `submit` (joueur) : auto-save de la réponse courante (écrase la précédente si déjà posée sur cette position).
