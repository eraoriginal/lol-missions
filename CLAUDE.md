...
| `npx tsx prisma/seeds/seed_quiz_ceo.ts` | Seed Quiz du CEO (1 question par type × 16 types, idempotent) |
...
| `npx tsx scripts/test-ranking-drag.ts` | Tests unitaires drag & drop ranking Quiz CEO (39 cas : offset, hover index, drop) |
...
lib/
  ...
  quizCeo/                       — config (16 types) · types (union payload/answer) · serialize (strip answer)
                                   ...
                                   anti-cheat : rankingDrag (helpers purs unit-testés)
...
- **16 types de questions** tirées parmi ceux que le créateur active dans le lobby :
  `image-personality`, `text-question`, `country-shape`, `expression`, `music`, `translation`, `multiple-choice`, `odd-one-out`, `lol-player-match`, `country-motto`, `brand-logo`, `absurd-law`, `price`, `who-said`, `ranking`, `lol-champion`.
- Timer configurable 10–300s (défaut 30s) dans le lobby. **Nombre de questions figé à 25** (cf. `QUESTION_COUNT_DEFAULT`) — non modifiable depuis le lobby. Tirage : 1 question random par type activé (max 16) + remplissage avec `text-question` jusqu'à 25, puis shuffle final pour intercaler les fillers.
...
3. **review** : le créateur navigue question par question, voit les réponses de tous + la solution, valide bon/faux pour chaque joueur. Pour `music`, validation séparée artiste / titre (split points via `splitMusicPoints(totalPoints)` : pour 3 pts → artiste=1, titre=2).
...
3. **Asset proxy opaque** sur `/api/games/quiz-ceo/[code]/asset/[index]` pour tous les payloads dont l'URL d'asset révèle la réponse. Catégories couvertes : `brand-logo`, `worldle`, `lol-champion` (splash + spells via `?slot=q|w|e|r|p`). Les URLs dans le payload exposé deviennent opaques (ex `/asset/3` au lieu de `/brand-logos/apple.svg`).
4. **Shuffle des `items` ranking** : dans `start/route.ts` on shuffle à la fois `items` ET `shuffledOrder` (sans ça `payload.items` était dans l'ordre de la réponse → cheater pouvait lire la solution dans les devtools).

En phase `review` / `leaderboard`, la partie est terminée : on expose tout.

**Quand ajouter un type** : si la nouvelle catégorie utilise un asset path qui révèle la réponse (ex `/foo/macron.jpg`), il faut **(a)** ajouter le préfixe à `ALLOWED_PREFIXES` dans `asset/[index]/route.ts`, **(b)** ajouter le préfixe à `SPOIL_PREFIXES` dans `rooms/[code]/route.ts`. Si payload contient plusieurs URLs d'assets distincts (cas spells), les exposer via `?slot=` query param.
...
- `start` (créateur) : tirage des 25 questions en 4 étapes (cf. JSDoc du handler) :
  1. 1 question random par type activé (max 16)
  2. Fill avec `text-question` jusqu'à 25
  ...
  - `ranking` : shuffle `items` ET `shuffledOrder` (sinon l'ordre canonique de la réponse leak via `payload.items`)
...
- `review-validate` (créateur) : bascule `validated`/`validatedArtist`/`validatedTitle`, recalcule `score` total à partir de toutes les réponses validées.
...
- `set-timer`, `toggle-type` (créateur, réglages lobby). La route `set-question-count` a été supprimée — le nombre est figé à 25.
...
5. **Rendu review** [`ReviewView.tsx`](app/games/quiz-ceo/components/ReviewView.tsx) : `case '<type>':` dans `QuestionDisplay`. Plus le type ajouté au groupe `CanonicalAnswer` selon la shape de l'answer (ex. `StringAnswer` → groupe avec `image-personality`, `text-question`, etc.).
...
7. **Anti-spoil (si nécessaire)** : si la catégorie expose des URLs d'assets nommés d'après la réponse, ajouter le préfixe à `ALLOWED_PREFIXES` (`asset/[index]/route.ts`) et `SPOIL_PREFIXES` (`rooms/[code]/route.ts`). Cf. section « Sécurité anti-spoil ».
...
- **DB-stored** (`text-question`, `expression`, `translation`, `country-motto`, `absurd-law`, `who-said`, `lol-player-match`, `multiple-choice`, etc.) : N rows en DB, le tirage est fait par la requête Prisma + le shuffling de `start`. Idéal quand chaque entrée a un `payload` + `answer` qui lui sont propres.

### Drag & drop ranking — pattern container-based
[`RankingInput`](app/games/quiz-ceo/components/QuestionPlayer.tsx) utilise une animation « live insertion » (les autres items se décalent pour faire de la place pendant le drag). Pour éviter le **feedback loop tremblant** entre `dragOver` per-row et le shift visuel, la détection du hover est faite **au niveau du container** : on calcule l'index logique via `Math.floor((cursorY - containerTop) / rowHeight)`. Le container ne bouge pas, donc le calcul est stable même quand les enfants ont des `transform: translateY`.

Helpers purs unit-testés : [`lib/quizCeo/rankingDrag.ts`](lib/quizCeo/rankingDrag.ts) (`computeRankingOffset`, `computeRankingHoverIndex`, `applyRankingDrop`). Tests : `npx tsx scripts/test-ranking-drag.ts` (39 cas, dont un test explicite anti-tremblement).
